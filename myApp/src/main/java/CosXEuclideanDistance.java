 ...
 * @author Haifeng Li
 *
 * We use the cos(latitude) in Euclidean distance: d = ( (x1-x2)^2 + (y1-y2)*cos((x1+x2)/2) ^ 2 )^(1/2)
 */
public class CosXEuclideanDistance implements Metric<double[]>, Serializable {
     ...
     */
    public CosXEuclideanDistance(double[] weight) {
    ...
    public double d(int[] x, int[] y) {
        ...
        if (x.length != 2)
            throw new IllegalArgumentException(String.format("Arrays length must be 2---(lat, lng)") );
